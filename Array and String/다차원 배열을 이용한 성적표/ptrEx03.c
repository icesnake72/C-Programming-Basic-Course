...
향후 버전2에서는 학번별 정렬(오름, 내림차순), 각 과목별 정렬, 성적순으로 정렬 기능을 추가할 예정!!!
또한 학번정보외에 학생 이름 데이터를 추가하고,
평균은 소숫점 2째자리까지 구해서 좀 더 정교한 데이터를 제공할 수 있도록 합니다.
최종적으로 구해진 정보를 엑셀에서 활용할 수 있도록 csv 파일로 저장할 수 있는 기능도 추가할 예정입니다.
*/
